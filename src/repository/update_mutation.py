...
async def update_tasks(
	id: Annotated[str, UUID], tasks: TasksUpdateGQL, info: Info
) -> TasksType:
	...
	if (_user := converted_data.get("user")) is not None and _user != (
		_ := await tasks_repository.get_entity_by_id(
			entity_id=id, db=info.context.db
		)
	).user:
...
async def list_tasks_update(
	id: Annotated[str, UUID], list_tasks: ListTasksUpdate, info: Info
) -> ListTaskType:
	...
	new_items: ListTaskGQLResponse = ListTaskGQLResponse.model_construct(**items_dict)
...
class UpdateMutation:
	...
	@strawberry.mutation
	async def update_tasks_mutation(
		self, id: Annotated[str, UUID], tasks: TasksUpdateGQL, info: Info
	) -> TasksType:
		return await update_tasks(id=id, tasks=tasks, info=info)

	@strawberry.mutation
	async def list_tasks_update(
		self, id: Annotated[str, UUID], list_tasks: ListTasksUpdate, info: Info
	) -> ListTaskType:
		return await list_tasks_update(id=id, list_tasks=list_tasks, info=info)
